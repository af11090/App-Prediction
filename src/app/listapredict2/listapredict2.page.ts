...
@Component({
  selector: 'app-listpredict2',
  templateUrl: './listapredict2.page.html',
  styleUrls: ['./listapredict2.page.scss'],
})
export class Listapredict2Page implements OnInit {
  ...
  obtenerDatos() {
    ...
    //   }
    // });
    this.http.get<any[]>('https://backendjs-dee6d131d346.herokuapp.com/api/registros2/2').subscribe({
      next: (data) => {
        const requests = data.map(item =>
  ...
  extraerResultado(Resultado: string): string {
    if (Resultado.includes('Tiene el tipo de anemia BTT')) {
      return 'BTT';
    } else if (Resultado.includes('Tiene el tipo de anemia AIF')) {
      return 'AIF';
    } else if (Resultado.includes('Tiene el tipo de anemia HbE')) {
      return 'HbE';
