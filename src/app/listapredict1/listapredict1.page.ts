...
@Component({
  selector: 'app-listpredict1',
  templateUrl: './listapredict1.page.html',
  styleUrls: ['./listapredict1.page.scss'],
})
export class Listapredict1Page implements OnInit {
  ...
  constructor(private http: HttpClient,private alertCtrl:AlertController,
    private service:Lista1Service,private toastController: ToastController) { }
        ...
        const requests = data.map(item =>
          this.http.get<any>(`http://localhost:3000/api/pacientes/${item.id_paciente}`).pipe(
            map(paciente => ({
              ...
              Fecha: new Date(item.Fecha).toLocaleDateString(),
              Resultado: item.Resultado ? (item.Resultado.includes('Si tiene anemia') ? 'Sí tiene' : 'No tiene') : 'No disponible'
            }))
          )
        );
